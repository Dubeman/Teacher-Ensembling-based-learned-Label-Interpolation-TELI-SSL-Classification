...
class ImageClassificationBaseMixup(nn.Module):
    ...
    def training_step_NeuralNetwork(self,model,val_loader, device):
      model.train()
      val_loss = []
    ...
    def training_step_deepbeta_no_distribution(self,student_model,images,label_1,label_2,alpha):
      #  lam = np.random.beta(alpha, alpha)
        preds = student_model(images)
        if label_2 is None:
          loss_deep_beta = F.cross_entropy(preds, label_1)
        
        else :
          loss_deep_beta = alpha*F.cross_entropy(preds,label_1)+(1-alpha)*F.cross_entropy(preds,label_2)
        return loss_deep_beta
    
    
    def training_step_deepbeta(self,student_model,images,label_1,label_2,alpha):
        lam = np.random.beta(alpha, alpha)
        preds = student_model(images)
        if label_2 is None:
          loss_deep_beta = F.cross_entropy(preds, label_1)
          ...
          loss_deep_beta = lam*F.cross_entropy(preds,label_1)+(1-lam)*F.cross_entropy(preds,label_2)
        return loss_deep_beta
    
    def training_step_normal(self,images,labels):
        
        out = self(images)                  # Generate predictions
        loss_normal = F.cross_entropy(out, labels) # Calculate loss
        return loss_normal
    
    def training_step_label_mixup(self,images,label_1,label_2,alpha):
    ...
    def training_step_total_mixup(self,images,label_1,label_2,alpha):
        if alpha > 0:
          lam = np.random.beta(alpha, alpha)
        else:
          lam = 1
...
#      else:
        ...
        y_a, y_b = y, y[index] 
        preds = self(images)
        loss = lam * F.cross_entropy(preds, label_1) + (1 - lam) * F.cross_entropy(preds, label_2)
        return loss
...
class MobileNet(ImageClassificationBaseMixup):
...
class WideResNet(ImageClassificationBaseMixup):
...
class VGG(ImageClassificationBaseMixup):
    def __init__(self, vgg_name, num_classes):
        super(VGG, self).__init__()
        self.features = self._make_layers(cfg[vgg_name])
        self.classifier = nn.Linear(512, num_classes) #100 for CIFAR
...
class Cifar10CnnModel_1(ImageClassificationBase):
    def __init__(self):
        super().__init__()
...
class Cifar10CnnModel_2(ImageClassificationBase):
...
class ResNet(ImageClassificationBaseMixup):
