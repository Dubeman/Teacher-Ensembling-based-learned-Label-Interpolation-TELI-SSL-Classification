...
from models import VGG,ResNet9, MobileNet,ResNet101,ResNet34, WideResNet,build_wideresnet, ResNet18,Cifar10CnnModel_2
...
parser.add_argument('--extract_cam', default= False, type=str, help='whether or not Grad-CAM plots are needed')
parser.add_argument('--load_ckp', default=  False, type=str, help='whether or not to resume training')
parser.add_argument('--learned_alpha', default=  False, type=str, help='whether or not to learn alpha')
parser.add_argument('--teacher_1', default= 'VGG-11' , type=str, choices=['Res-18', 'MobileNet','MAE','VGG-11','VGG-13'], help='Pre-trained teacher 1 (on relevant dataset)')
parser.add_argument('--teacher_2', default=  'VGG-13', type=str,choices=['Res-18', 'MobileNet','MAE','VGG-11','VGG-13'], help='Pre-trained teacher 2 (on relevant dataset)')
...
parser.add_argument('--eval-step', default=400, type=int, help='number of eval steps to run')
...
def cam_extractor(student_model,dataset):
  # get the softmax weight
    ...
    # show and save the results
          show_cam(CAMs, 32, 32, image.permute(1,2,0).numpy(), class_idx, save_name)
          count+= 1
...
def fit_baseline( device,unsupervised_epochs,supervised_epochs, max_lr, model,model_1, labeled_train_loader,unlabeled_loader, val_loader,save_path, 
 ...
 #                                               steps_per_epoch=len(train_loader))
    ...
    supervised_phase(supervised_epochs,model,labeled_train_loader,val_loader,optimizer,s_scheduler,history,lrs,train_losses)
    ...
    for epoch in range(unsupervised_epochs):
        ...
        for (images_us,_),_ in unlabeled_loader:  # Total no of batches per epoch
      ...
      #    weight_u = args.lambda_u * min(1., (i + 1) / args.uda_steps) 
         # loss_uda = weight_u * t_loss_u #UDA Loss
          ...
          loss_normal = model.training_step_normal(images_us, hard_pseudo_label_1)
             
          train_losses.append(loss_normal.reshape([]))
          ...
          if epoch%2 == 0 and args.extract_tsne == True:
        ...
        if epoch%2 == 0 and args.extract_tsne == True:
          tsne = TSNE(n_components=2).fit_transform(f)
          targets = pd.DataFrame(labeled_dataset.targets)
          targets= targets.astype(int)
          plot_features(tsne,targets,epoch)
    ...
    for epoch in range(unsupervised_epochs):
      ...
      #    weight_u = args.lambda_u * min(1., (i + 1) / args.uda_steps) 
         # loss_uda = weight_u * t_loss_u #UDA Loss
            ...
            print(matched_images.shape)
            print(mismatched_images.shape)
          ...
          with torch.cuda.amp.autocast():
            if matched_images.shape[0] == 0:
             loss_mixup = model.training_step_label_mixup(mismatched_images, mixup_labels_1,mixup_labels_2,args.alpha)
            # loss_mixup = model.training_step_label_mixup(mismatched_inputs, mixup_labels_1,mixup_labels_2,alpha)
             ...
             loss_normal = model.training_step_normal(matched_images, matched_labels) 
...
def supervised_phase(supervised_epochs,model,labeled_train_loader,val_loader,optimizer,s_scheduler,history,lrs,train_losses):
...
if __name__ == '__main__':
     ...
     #   model_2 =  resnet50(num_classes=10, dropout = False).to(device)
        ...
        if args.teacher_1 == 'MAE' and args.teacher_2 == 'VGG-13' and args.dataset == 'cifar10':
          
          model_1 = torch.load('/content/gdrive/MyDrive/Psuedo-Mixup/teacher models/MAE_cifar_10_31epochs.pt',map_location=torch.device('cuda'))
        
          model_2 = VGG('VGG13',10).to(device) 
          model_2.load_state_dict(torch.load('/content/gdrive/MyDrive/Psuedo-Mixup/teacher models/model_1_cifar10',map_location=torch.device('cuda')))
          
          student_model = resnet18(10,3).to(device)


        if args.teacher_1 == 'VGG-11' and args.teacher_2 == 'VGG-13' and args.dataset == 'cifar100':
          
          model_1 = VGG('VGG11',100).to(device)
          model_1.load_state_dict(torch.load('/content/gdrive/MyDrive/Psuedo-Mixup/teacher models/model_2_cifar100',map_location=torch.device('cuda')))
          
          model_2 = VGG('VGG13',100).to(device) 
          model_2.load_state_dict(torch.load('/content/gdrive/MyDrive/Psuedo-Mixup/teacher models/model_1_cifar100',map_location=torch.device('cuda')))
          
          student_model = MobileNet(3,100).to(device)  
          
        
        if args.teacher_1 == 'VGG-13' and args.teacher_2 == 'VGG-11' and args.dataset == 'SVHN':
          model_1 = VGG('VGG13',10).to(device)
          model_1.load_state_dict(torch.load('/content/gdrive/MyDrive/Psuedo-Mixup/teacher models/model_1_svhn',map_location=torch.device('cuda')))
       
          model_2 = VGG('VGG11',10).to(device)
          model_2.load_state_dict(torch.load('/content/gdrive/MyDrive/Psuedo-Mixup/teacher models/model_2_svhn',map_location=torch.device('cuda')))
       
          student_model = resnet34(num_classes=10,in_channels = 3).to(device)

        
        if args.teacher_1 == 'VGG-11' and args.teacher_2 == 'VGG-13' and args.dataset == 'cifar10':
          model_1 = VGG('VGG11',10).to(device)
        ...
        save_path = None # CHANGE THE SAVEPATH TO LOCATION OF SAVING IF YOU WANT THE MODEL TO BE SAVED
    
        
        if args.extract_cam:
          features_blobs = []
          student_model.layer4[2].conv2.register_forward_hook(hook_feature) # this is for Resnet-34 only, for others you have to place the hook appropriately
          cam_extractor(student_model,test_dataset)

       
        if args.extract_tsne:
          ...
          student_model.layer4[2].conv2.register_forward_hook(get_features('feats'))  # this is for Resnet-34 only, for others you have to place the hook appropriately      
        #  student_model.layer.register_forward_hook(get_features('feats'))
          for images,labels in test_loader:
            print(images.shape)
            break
          out = student_model(images.to(device))
          FEATS.append(features['feats'].cpu().numpy())
          f = np.concatenate(FEATS)
          print(f.shape)
          tsne = TSNE(n_components=2).fit_transform(f)
          targets = pd.DataFrame(labels.numpy())
          targets= targets.astype(int)
          plot_features(tsne,targets,1)
      

      
        save_path = '/content/gdrive/MyDrive/Psuedo-Mixup/Student models/400MNIST_Mobilenet_res18_baseline.pth'
      #  if os.path.exists(save_path):
      #    args.load_ckp =  True

        history = [evaluate(student_model, finetune_loader)]
        
        history += fit_baseline(device = device,unsupervised_epochs =30,supervised_epochs = 30, max_lr = max_lr,model = student_model,model_1 = model_1, labeled_train_loader = labeled_loader,unlabeled_loader = unlabeled_loader, val_loader = finetune_loader, 
                weight_decay=0, grad_clip=None, opt_func=opt_func,save_path = save_path)
      #  history += fit_one_cycle(device = device,unsupervised_epochs =30,supervised_epochs = 0, max_lr = max_lr,model = student_model,model_1 = model_1,model_2 = model_2, labeled_train_loader = labeled_loader,unlabeled_loader = unlabeled_loader, val_loader = finetune_loader, 
      #          weight_decay=0, grad_clip=None, opt_func=opt_func,save_path = save_path)
        ...
        print(evaluate(model_2,test_loader))
        print(evaluate(model_1,test_loader))
        test = evaluate(student_model,test_loader)
        print(test)
  
  #      student_model = ResNet9(3,100) 
        #student_model.load_state_dict(torch.load('/content/drive/MyDrive/CIFAR100-resnet9-VGG11-VGG13.pth',map_location=torch.device('cpu')))
